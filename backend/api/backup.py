...
def _sqlite_snapshot(source: Path, target: Path) -> None:
    ...
    Copying only the main database file can lose committed pages that still live
    in a WAL file.  SQLite's backup API reads a consistent database view and is
    safe while normal readers are connected.
    """
    target.parent.mkdir(parents=True, exist_ok=True)
    with (
        sqlite3.connect(str(source), timeout=30) as source_conn,
        sqlite3.connect(str(target), timeout=30) as target_conn,
    ):
        source_conn.backup(target_conn)
        target_conn.commit()


def _switch_sqlite_database(source: Path, target: Path, *, temp_suffix: str) -> None:
    """Switch ``target`` to ``source``, with a Windows-safe SQLite fallback.

    The same-filesystem replace remains the fast path.  On Windows an unrelated
    reader may briefly retain a file handle even after the async engine is
    disposed; in that case the SQLite backup API updates the live database
    consistently without replacing its directory entry.
    """
    target.parent.mkdir(parents=True, exist_ok=True)
    candidate = target.with_suffix(temp_suffix)
    ...
    try:
        ...
        logger.warning(
            "SQLite atomic replace blocked by an open Windows handle; "
            "using the online backup fallback"
        )
    finally:
        # On a successful replace the candidate no longer exists.
    ...
    with (
        ...
        if result and result[0] != "ok":
            raise RuntimeError(f"SQLite switch verification failed: {result[0]}")
...
async def restore_backup(file: UploadFile):
    ...
    await task_manager.pause_for_maintenance()
    ...
    try:
        ...
        if db_path.exists():
            _sqlite_snapshot(db_path, rollback_dir / "rag_agent.db")
        ...
        # Atomic SQLite replacement with a Windows open-handle fallback.
        _switch_sqlite_database(staging_db, db_path, temp_suffix=".restore_tmp")
        ...
        try:
            await task_manager.resume_after_maintenance()
        finally:
            mstate.release()
...
async def _do_rollback(
    ...
    if rollback_db.exists():
        _switch_sqlite_database(rollback_db, db_path, temp_suffix=".rollback_tmp")
