...
sys.path.insert(0, str(Path(__file__).resolve().parent))  # tests dir for evaluate_rag import

from evaluate_rag import QueryCase, TestDoc, save_results
from qrels_schema import QrelDataset, document_key_from_filename

from config import settings
from rag.splitter import split_text

DATA_DIR = Path(__file__).resolve().parent / "eval_data"
QRELS_PATH = Path(__file__).resolve().parent / "qrels_complex_v2.json"
...
def validate_complex_qrels() -> QrelDataset:
    """Fail fast when reviewed qrels drift from the corpus or query list."""
    dataset = QrelDataset.load(str(QRELS_PATH))
    docs = load_docs()
    expected_queries = {case.query for case in QUERY_CASES}
    actual_queries = {item.query for item in dataset.queries}

    if len(actual_queries) != len(dataset.queries):
        raise ValueError("Complex qrels contain duplicate query text")
    if actual_queries != expected_queries:
        ...
        raise ValueError(f"Complex qrels query mismatch: missing={missing}, extra={extra}")

    corpus_sections: dict[str, dict[str, str]] = {}
    for doc in docs:
        document_key = document_key_from_filename(doc.filename)
        corpus_sections[document_key] = {
            chunk.section_key: chunk.text
            for chunk in split_text(doc.content, settings.chunk_size, settings.chunk_overlap)
        }

    errors: list[str] = []
    for query in dataset.queries:
        if not query.relevant:
            errors.append(f"{query.query_id}: no relevant passages")
            continue
        relevant_texts: list[str] = []
        for item in query.relevant:
            sections = corpus_sections.get(item.document_key)
            if sections is None:
                errors.append(f"{query.query_id}: unknown document_key={item.document_key}")
                continue
            text = sections.get(item.section_key)
            if text is None:
                errors.append(
                    f"{query.query_id}: unknown section={item.document_key}#{item.section_key}"
                )
                continue
            relevant_texts.append(text.lower())

        if query.expected_answer_facts and relevant_texts:
            joined = "\n".join(relevant_texts)
            missing_facts = [
                fact for fact in query.expected_answer_facts
                if not any(variant.strip().lower() in joined for variant in fact.split("|"))
            ]
            if missing_facts:
                errors.append(f"{query.query_id}: facts absent from qrels={missing_facts}")

    if errors:
        raise ValueError("Invalid complex qrels:\n  - " + "\n  - ".join(errors))
    return dataset


# 29 queries designed for disambiguation
...
if __name__ == "__main__":
    # Pin the reviewed complex-benchmark retrieval profile.
