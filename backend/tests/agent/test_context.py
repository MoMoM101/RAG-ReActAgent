...
class TestTrimMessages:
    ...
    def test_duplicate_content_uses_message_identity_for_dropped_tracking(self):
        cm = ContextManager(max_tokens=40)
        messages = [
            ChatMessage(role="user", content="same question", message_id="old-id"),
            ChatMessage(role="assistant", content="a very long answer " * 30),
            ChatMessage(role="user", content="same question", message_id="new-id"),
        ]

        trimmed, _, dropped = cm.trim_messages(messages)

        assert trimmed[-1].message_id == "new-id"
        assert dropped == ["same question"]
        assert cm.last_dropped_message_ids == ["old-id"]
