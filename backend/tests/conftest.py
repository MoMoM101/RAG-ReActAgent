...
TEST_JWT_SECRET = "test-jwt-secret-that-is-longer-than-32-characters"

os.environ.setdefault("JWT_SECRET", TEST_JWT_SECRET)
os.environ.setdefault("BOOTSTRAP_ADMIN_USERNAME", TEST_ADMIN_USERNAME)
...
def admin_headers() -> dict[str, str]:
    """Return a system-administrator Bearer token for protected routes."""
    from auth.jwt import create_access_token

    token = create_access_token("test-admin-id", TEST_ADMIN_USERNAME, "system_admin")
    return {"Authorization": f"Bearer {token}"}


@pytest.fixture(autouse=True)
def reset_rate_limiter():
    """Keep per-process rate-limit counters isolated between tests."""
...
@pytest_asyncio.fixture
async def bootstrap_admin(setup_db, monkeypatch):
    """Create the bootstrap admin user (lifespan doesn't run under ASGITransport)."""
    from config import settings
    from main import _bootstrap_user

    monkeypatch.setattr(settings, "bootstrap_admin_username", TEST_ADMIN_USERNAME)
    monkeypatch.setattr(settings, "bootstrap_admin_password", TEST_ADMIN_PASSWORD)
