...
ADMIN_HEADERS = {
    ...
    + create_access_token("restore-admin-id", "restore-admin", "system_admin")
}
...
def test_sqlite_switch_falls_back_when_windows_blocks_replace(tmp_path):
    """An open Windows handle must not make restore/rollback impossible."""
    from api.backup import _switch_sqlite_database

    source = tmp_path / "source.db"
    target = tmp_path / "target.db"
    for path, value in ((source, "restored"), (target, "original")):
    ...
    with patch.object(Path, "replace", side_effect=PermissionError("in use")):
        _switch_sqlite_database(source, target, temp_suffix=".restore_tmp")

    with sqlite3.connect(target) as conn:
        assert conn.execute("SELECT value FROM state").fetchone()[0] == "restored"
        assert conn.execute("PRAGMA quick_check").fetchone()[0] == "ok"
    assert not target.with_suffix(".restore_tmp").exists()
