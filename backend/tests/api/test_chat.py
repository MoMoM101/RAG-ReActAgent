...
def test_answer_saved_extraction_is_durable_and_idempotent():
    manager = MagicMock()
    registry = MagicMock()
    with (
        patch("worker.tasks.get_task_manager", return_value=manager),
        patch("worker.tasks.get_handler_registry", return_value=registry),
    ):
        _schedule_session_extraction(
            "conversation-1",
    ...
    assert kwargs["task_type"] == "extract_session_memories"
    assert kwargs["payload"] == {
        "conversation_id": "conversation-1",
        "delay_seconds": 2,
    }
    assert kwargs["idempotency_key"] == (
        "session-memory:v1:conversation-1:message-1"
    )
