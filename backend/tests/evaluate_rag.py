...
def save_results(results: dict, filepath: str = "evaluation_results_v2.json") -> str:
    ...
    # Keep ranked evidence in the artifact so a regression can be attributed
    # to retrieval, fusion, or reranking without rerunning external models.
    qrels_per_query = results.get("qrels_per_query", [])
    strategy_results = results.get("strategy_results", {})
    diagnostics = []
    for query_index, qc in enumerate(results.get("query_cases", [])):
        qrels = qrels_per_query[query_index] if query_index < len(qrels_per_query) else []
        expected = [
            f"{qrel.document_key}#{qrel.section_key}"
            for qrel in qrels
        ]
        ranked_by_strategy = {}
        for strategy, strategy_data in strategy_results.items():
            ...
            ranked_lists = strategy_data.get("results", [])
            ranked = ranked_lists[query_index] if query_index < len(ranked_lists) else []
            ranked_by_strategy[strategy_name] = [
                {
                    "rank": rank,
                    ...
                    "source": item.source,
                    "matched": any(
                        item.document_key == qrel.document_key
                        and (
                            not qrel.section_key
                            or not item.section_key
                            or item.section_key == qrel.section_key
                        )
                        for qrel in qrels
                    ),
...
def _reviewed_qrels_by_query(
    ...
    *,
    ...
    """Index qrels and enforce complete coverage in strict evaluations."""
    indexed = {item.query: item for item in dataset.queries}
    if len(indexed) != len(dataset.queries):
        raise ValueError(f"Duplicate query text in qrels dataset: {dataset_path}")

    missing = [case.query for case in query_cases if case.query not in indexed]
    if missing and not allow_fallback:
        preview = ", ".join(repr(query) for query in missing[:5])
        suffix = "" if len(missing) <= 5 else f" (+{len(missing) - 5} more)"
        raise ValueError(
            f"Strict qrels coverage failed for {dataset_path}: "
            f"{len(missing)} missing queries: {preview}{suffix}"
        )
    return indexed
...
def _corpus_sha256(documents: list[TestDoc]) -> str:
...
async def run_single_strategy(
    ...
    top_k: int = 10,
    document_id: str = "",
    allowed_document_keys: set[str] | None = None,
) -> tuple[list, int]:
    ...
    t0 = _time.time()
    candidate_k = max(top_k, 50) if allowed_document_keys else top_k

    if strategy == AblationStrategy.KEYWORD_ONLY:
        fts = BM25Search()
        text_results = await fts.search(
        ...
        vector_results = await vectordb.search(query_vector, top_k=candidate_k)
        results = [
            RetrievalResult(
                chunk_id=r.chunk_id, document_id=r.document_id,
        ...
        results = list(await hybrid_search(
            query, top_k=candidate_k, use_rerank=False
        ))

    elif strategy == AblationStrategy.HYBRID_RERANK:
        results = list(await hybrid_search(
            query, top_k=candidate_k, use_rerank=True
        ))
    ...
    if allowed_document_keys is not None:
        results = [
            ...
            if result.document_key in allowed_document_keys
        ]
...
async def run_evaluation(
    *,
    ...
    allow_qrels_fallback: bool = True,
    persist_results: bool = True,
    results_filename: str = "evaluation_results_v2.json",
):
    print("=" * 70)
    print("RAG 检索精准度评测")
    ...
    async with session_scope() as session:
        ...
        vectordb = await create_vectordb() if existing_docs else None
        fts = BM25Search()
    ...
    for i, doc_def in enumerate(TEST_DOCS):
        ...
        try:
            ...
            print(f"   [{i+1}/{len(TEST_DOCS)}] {doc_def.filename} -> {doc_id[:8]}... [ok]")
        except ValueError as exc:
            if "already exists (hash matched)" not in str(exc):
                raise
    ...
    resolved_qrels_path = Path(qrels_path) if qrels_path else (
        Path(__file__).resolve().parent / "qrels_data_v2.json"
    )
    qrels_dataset = QrelDataset.load(str(resolved_qrels_path))
    qrels_by_query = _reviewed_qrels_by_query(
        qrels_dataset,
        QUERY_CASES,
        resolved_qrels_path,
        allow_fallback=allow_qrels_fallback,
    )
    ...
    allowed_document_keys = {
        _stable_doc_key(document.filename) for document in TEST_DOCS
    }
    for j, qc in enumerate(QUERY_CASES):
        qrels = qrels_per_query[j]

        for strategy in strategies:
            results, lat = await run_single_strategy(
                strategy,
                qc.query,
    ...
    hit5 = agg["hit"][5]
    ndcg5 = agg["ndcg"][5]
    # Precision@5 has a hard ceiling of 20% for single-answer queries, so it
    # cannot grade a mixed benchmark fairly. Rank quality and hit rate can.
    if mrr >= 0.85 and ndcg5 >= 0.80 and hit5 >= 0.90:
        rating = "A — 优秀"
    elif mrr >= 0.70 and ndcg5 >= 0.65 and hit5 >= 0.80:
        rating = "B — 良好"
    elif mrr >= 0.50 and hit5 >= 0.65:
        rating = "C — 一般"
    else:
        rating = "D — 需要优化"
    ...
    evaluation_results = {
        ...
        "qrels_per_query": qrels_per_query,
        "doc_ids": doc_ids,
        "strategy_results": strategy_results,
        "qrels_fallback_count": fallback_qrels_count,
        "provenance": {
            "qrels_file": resolved_qrels_path.name,
            "qrels_sha256": _file_sha256(resolved_qrels_path),
            "corpus_sha256": _corpus_sha256(TEST_DOCS),
        },
    }

    # ── Persist results ──
    results_file = (
        save_results(evaluation_results, results_filename)
        if persist_results
        else "(not persisted)"
    )
