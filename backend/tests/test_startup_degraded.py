...
async def test_health_dependencies_returns_all_components():
    ...
    with (
        ...
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            ...
            assert set(data["optional_models"]) == {"ocr", "reranker"}
...
@pytest.mark.asyncio
async def test_optional_model_failure_is_degraded_not_core_error():
    from main import app
    ...
    with patch("ocr.factory.get_ocr_status", return_value=failed):
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            response = await client.get("/api/health/dependencies")
    ...
    assert response.status_code == 200
    data = response.json()
    assert data["dependencies"]["ocr"] == "failed"
    assert data["status"] == "degraded"
    assert data["core_ready"] is True
