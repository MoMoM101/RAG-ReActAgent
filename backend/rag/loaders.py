...
class OCRModelNotReadyError(RuntimeError):
    """OCR is enabled and still downloading/loading in the background."""


class OCRModelUnavailableError(RuntimeError):
    """OCR was requested but its dependency/model failed to load."""


def _ready_ocr() -> Any:
    from ocr.factory import create_ocr, get_ocr_status, is_ocr_ready

    engine = create_ocr()
    status = get_ocr_status()
    if engine is not None and is_ocr_ready():
        return engine
    if status["status"] in ("failed", "missing_dependency"):
        detail = status.get("last_error") or "未知错误"
        raise OCRModelUnavailableError(
            "OCR 模型不可用；可运行 `python -m tools.download_models --ocr` 手动安装，"
            f"或在设置中关闭 OCR。原因: {detail}"
        )
    raise OCRModelNotReadyError("OCR 模型仍在后台下载或加载，文档将在模型就绪后自动继续处理")
    ...
    for page in doc:
        ...
        if settings.ocr_enabled:
            try:
                ...
                # Embedded images are optional when useful PDF text already
                # exists, so skip them while OCR is still downloading.
                if get_ocr_status()["status"] == "ready":
                    ocr = _ready_ocr()
            ...
            except (OCRModelNotReadyError, OCRModelUnavailableError):
                pass
            except Exception:
