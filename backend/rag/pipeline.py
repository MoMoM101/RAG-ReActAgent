            ...
            async with sem:
                ...
                for attempt in range(settings.ingestion_max_retries):
                    try:
                        ...
                        logger.info("ingestion waiting for OCR doc_id=%s", doc_id)
                        return
                    except Exception as e:
...
async def _mark_waiting_for_ocr(doc_id: str, message: str) -> None:
    """Persist a stable, non-failure state while the optional model loads."""
    from rag.progress import progress
    ...
    async with session_scope() as session:
        await session.execute(
            ...
            .values(status=DocStatus.waiting_for_ocr, error_message=message[:500])
        )
        await session.commit()
    progress.publish(doc_id, {
        "status": "waiting_for_ocr",
        "message": "OCR 模型正在后台准备，完成后将自动继续处理",
    })


async def resume_waiting_for_ocr_documents() -> int:
    """Atomically claim and reschedule documents after OCR becomes ready."""
    async with session_scope() as session:
        result = await session.execute(
            select(Document).where(Document.status == DocStatus.waiting_for_ocr)
        )
        documents = list(result.scalars().all())
        recoverable: list[tuple[str, str, str, str]] = []
        storage = get_storage()
        for doc in documents:
            if not doc.storage_key or not await storage.exists(doc.storage_key):
                doc.status = DocStatus.failed
                doc.error_message = "OCR 就绪后无法继续：原始文件不存在"
                continue
            ...
            if claimed.rowcount == 1:
                recoverable.append((doc.id, doc.filename, doc.storage_key, doc.file_type))
        await session.commit()

    scheduled = 0
    for doc_id, filename, storage_key, file_type in recoverable:
        await _run_document_ingestion(doc_id, filename, storage_key, file_type, background=True)
        scheduled += 1
    return scheduled
