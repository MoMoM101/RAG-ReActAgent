...
_CONTRASTIVE_SCORE_SPREAD = 0.15  # Top results <15% apart → needs contrastive re-rank


def _contrastive_disambiguate(
    results: list[RetrievalResult],
    signals,
    top_k: int,
) -> list[RetrievalResult]:
    """Phase 4: Re-rank when top results have close scores from different docs.

    If the top-k window has chunks from multiple documents whose best scores
    are within _CONTRASTIVE_SCORE_SPREAD of each other, boost chunks from
    the document whose chunks collectively match more disambiguation signals.
    """
    if len(results) < 2:
        return results

    # Only check the top window
    window = results[: top_k * 2]
    if len(window) < 3:
        return results

    # Check if top 2 are from different docs and close in score
    top_score = window[0].score
    second_score = window[1].score
    if top_score == 0 or window[0].document_id == window[1].document_id:
        return results

    score_spread = (top_score - second_score) / top_score
    if score_spread > _CONTRASTIVE_SCORE_SPREAD:
        return results  # Enough separation, no need

    # Scores are close — boost the document with more signal hits
    from rag.disambiguator import count_signal_hits

    # Aggregate signal hits per document in the window
    doc_signal_hits: dict[str, int] = {}
    for r in window:
        doc_signal_hits[r.document_id] = max(
            doc_signal_hits.get(r.document_id, 0),
            count_signal_hits(r.text, signals),
        )

    if not doc_signal_hits or max(doc_signal_hits.values()) == 0:
        return results

    # Boost chunks from the best-matching doc
    best_doc = max(doc_signal_hits, key=lambda d: doc_signal_hits[d])
    best_hits = doc_signal_hits[best_doc]
    if best_hits == 0:
        return results

    for r in results:
        if r.document_id == best_doc:
            r.score *= 1.0 + 0.05 * best_hits
...
async def hybrid_search(query: str, top_k: int | None = None, document_id: str = "",
    ...
    if settings.query_rewrite_enabled:
        should_rewrite = True
        if settings.query_rewrite_disambiguation_only:
            from rag.query_classifier import QueryType, classify_query
            qtype = classify_query(query)
            should_rewrite = qtype in (
                QueryType.AMBIGUOUS_SHORT,
                QueryType.AMBIGUOUS_CROSS_DOMAIN,
                QueryType.SHORT_KEYWORD,
            )
        if should_rewrite:
            try:
    ...
    # ── Phase 2: Disambiguation signal bonus ──
    from rag.disambiguator import apply_signal_bonus, extract_disambiguation_signals
    signals = extract_disambiguation_signals(query)
    # Only activate when 2+ distinct signal types present (higher confidence)
    signal_types = sum(
        bool(v) for v in (
            signals.currency_hints,
            signals.number_constraints,
            signals.entity_names,
            signals.standard_refs,
        )
    )
    if signals.has_signals() and signal_types >= 2:
        results = apply_signal_bonus(results, signals)
        # ── Phase 4: Contrastive re-rank on close scores across docs ──
        results = _contrastive_disambiguate(results, signals, top_k)
