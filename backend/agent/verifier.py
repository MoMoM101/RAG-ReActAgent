...
def grounding_repair_instruction(answer: str) -> str:
    ...
    return (
        ...
        "再逐条回答，不能整体拒答；其余部分最后写“无法确认：……”。每个列表项只写一个"
...
def select_better_grounded_answer(
    ...
    """Keep a repair only when deterministic grounding quality improves."""
    if not repaired.strip():
        return original

    def quality(answer: str) -> tuple[float, float, float, int]:
    ...
    return repaired if quality(repaired) > quality(original) else original
