...
async def create_vectordb() -> BaseVectorDB:
    ...
    # Multiple background ingestions can reach first-use initialization at the
    # same time. ensure_collection tolerates the create/create race by checking
    # that the collection exists after a conflicting create request.
    await db.ensure_collection(current_dim)
