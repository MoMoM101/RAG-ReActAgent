...
class MetricsCollector:
    ...
    def __init__(self):
        ...
        self.context_compactions: int = 0
        self.context_compacted_messages: int = 0
        self.memory_extractions: dict[str, int] = defaultdict(int)
        ...
        self.llm_prompt_tokens_total: int = 0
        self.llm_completion_tokens_total: int = 0
        self.llm_estimated_requests: int = 0
    ...
    def record_context_compaction(self, message_count: int):
        with self._lock:
            self.context_compactions += 1
            self.context_compacted_messages += max(0, message_count)

    def record_memory_extraction(self, outcome: str):
        with self._lock:
            self.memory_extractions[outcome] += 1
    ...
    def record_llm_usage(
        ...
        *,
        ...
        with self._lock:
            ...
            self.llm_estimated_requests += int(estimated)
            self.llm_requests += 1
        ...
        with self._lock:
            return {
                ...
                "agent": {
                    ...
                    "context_compacted_messages": self.context_compacted_messages,
                    "memory_extractions": dict(self.memory_extractions),
                },
                "tools": {
                    name: {
                ...
                "llm": {
                    ...
                    "prompt_tokens": self.llm_prompt_tokens_total,
                    "completion_tokens": self.llm_completion_tokens_total,
                    "estimated_requests": self.llm_estimated_requests,
...
def export_prometheus() -> str:
    ...
    lines.append(f'llm_prompt_tokens_total {snap["llm"]["prompt_tokens"]}')
    lines.append(f'llm_completion_tokens_total {snap["llm"]["completion_tokens"]}')
    lines.append(f'llm_usage_estimated_requests_total {snap["llm"]["estimated_requests"]}')
