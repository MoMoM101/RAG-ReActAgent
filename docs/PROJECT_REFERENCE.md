...
> 面向版本：`v0.2.1-beta`（发布标签 `v0.2.1-beta.3`）  
> 内容整理日期：2026-08-26  
...
重新评分只能更新确定性验证器或评测器产生的评分字段，必须保留原始回答、模型调用和延迟记录，并显式更新 rescored provenance，不能伪装成新的模型生成。数据集、提示词、模型、检索或生成链路变化后，应重新运行完整在线评测。
