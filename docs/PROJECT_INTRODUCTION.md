...
> 项目阶段：`v0.2.1-beta`（发布标签 `v0.2.1-beta.3`）  
> 核心定位：将本地文档转化为可检索、可引用、可验证的智能问答知识库。

![Python](https://img.shields.io/badge/Python-3.12-3776AB?logo=python&logoColor=white)
![React](https://img.shields.io/badge/React-19-61DAFB?logo=react&logoColor=black)
![License](https://img.shields.io/badge/License-MIT-green)
![Faithfulness](https://img.shields.io/badge/Faithfulness-100.00%25-brightgreen)
![Citation](https://img.shields.io/badge/Citation_Precision-99.62%25-brightgreen)
...
| 维度 | 当前结果 | 说明 |
|---|---:|---|
| 回答忠实度 | **100.00%** | 回答中的事实声明可由证据支持 |
| 引用精确率 | **99.62%** | 引用来源能够支持对应声明 |
| 引用召回率 | **99.62%** | 需要引用的声明获得有效引用 |
| 拒答准确率 | **100.00%** | 无充分证据时正确拒答 |
| 回答完成度 | **100.00%** | 对可回答问题提供完整结果 |
| 预期事实召回 | **85.21%** | 预先标注的关键事实被回答覆盖 |
| 检索 Recall@10 | **99.57%** | 严格 qrels 检索集 |
| 检索 Hit@3 | **100.00%** | 前 3 条结果至少命中一条相关证据 |
| 检索 MRR | **97.70%** | 首条相关结果排名表现 |
| 后端代码覆盖率 | **73.13%** | 11,227 个可执行语句，含分支覆盖统计 |
...
| 指标 | 控制组 | 优化组 | 提升 |
|---|---:|---:|---:|
| 忠实度 | 72.35% | **100.00%** | **+27.65 pp** |
| 拒答准确率 | 44.44% | **100.00%** | **+55.56 pp** |
| 预期事实召回 | **85.31%** | 85.21% | -0.10 pp |
| 回答完成度 | 80.65% | **100.00%** | **+19.35 pp** |
| 平均总延迟 | 3,465.77 ms | **1,990.28 ms** | **-42.57%** |

这组结果表明：optimized 组显著提升了回答忠实度、完成度和拒答准确率，并将平均总延迟降低 **42.57%**。正式报告保留最后一次在线运行的真实 draft/TTFT；仅对两个未产生输出的超时修复阶段按当前确定性策略进行了显式标记的本地 replay。
...
> “100% 已执行测试通过率”“73.13% 代码覆盖率”和 RAG 质量百分比是三种不同口径：前者反映本次离线选择集的执行结果，覆盖率反映代码被测试触达的范围，RAG 指标反映检索与回答质量，不能互相替代。
...
- 当前定位是单实例、本地优先的开源项目，不等同于完整 SaaS 多租户平台；
