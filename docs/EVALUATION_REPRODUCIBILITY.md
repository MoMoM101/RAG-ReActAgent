...
正式报告记录数据集、评估器、校验器、提示词或语料的 SHA-256。文本文件在计算 provenance 前统一将 CRLF/CR 换行规范化为 LF，确保 Windows 与 Linux 检出内容得到相同哈希。检索评测要求 `qrels_fallback_count=0`。这些措施可以发现文件漂移，但不能消除单人标注偏差；当前项目没有多人标注一致性数据。
...
据此，100.00% 的回答忠实度应解释为：

> 当前代码和提示词在固定合成基准上的受控在线评测结果。

不应解释为：

> 对任意真实文档或未见查询都能达到 100.00%。
...
当变化仅限于确定性验证器或评测器时，可以对报告中保留的原始回答做本地重评分；重评分必须保留原始模型输出、模型调用和延迟记录，并更新 rescored provenance。数据集、提示词、模型、检索或生成链路发生变化时，必须重新运行完整在线评测。

真实 Docker E2E 在受控本地环境运行，DashScope 密钥不上传到 GitHub。验收脚本输出脱敏 JSON，记录完整提交、12 个阶段和排除证据文件自身后的 Git 跟踪树摘要；标签工作流重新计算摘要并要求证据不超过 7 天。证据提交之后若任何源码、依赖或工作流文件变化，摘要门禁都会阻止发布。
...
- `comparison-003` 询问 Django REST Framework 与 FastAPI 哪个更好；
- 资料只支持两者各自的直接事实，没有完整支持易用性、社区或综合优劣结论；
- 当前版本输出带引用的两侧证据并明确保留比较结论，忠实度为 100%，但该样本预期事实召回为 20%；
- 这体现了系统在“补全预期事实”和“避免无证据推断”之间偏向后者。

此外，optimized 组有 24 条记录的预期事实召回未达到 100%，总体 expected fact recall 为 85.21%。这说明 99.62% 的引用指标不能替代事实覆盖率，后续优化应单独追踪跨文档、比较、否定和部分可回答问题的遗漏。

最后一次完整在线运行保留真实 draft、TTFT 和外部模型调用记录。`multi-hop-001` 与 `multi-hop-004` 的 LLM 修复在超时前没有产生替换文本，正式报告仅对这两个本地后处理阶段使用当前确定性策略 replay；每条记录均包含 `latency_replayed`、`latency_replay_reason`，报告顶层 `performance_replay` 保存范围和 query id。该 replay 不重新生成模型文本，也不替换供应商 draft 耗时。
