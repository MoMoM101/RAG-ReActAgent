...
const LEGACY_REFRESH_TOKEN_KEY = "rag_refresh_token";
...
function readAccessToken(): string | null {
  // Remove refresh tokens left by versions that stored them in JavaScript storage.
...
export const useAuthStore = create<AuthState>((set) => ({
  ...
  changePasswordAtLogin: async (
    ...
    if (!loginResponse.ok) throw new Error(await errorDetail(loginResponse));
    ...
    if (!changeResponse.ok) throw new Error(await errorDetail(changeResponse));

    const data = (await changeResponse.json()) as LoginResponse;
    persistAccessToken(data.access_token);
    set({
      accessToken: data.access_token,
      user: data.user,
