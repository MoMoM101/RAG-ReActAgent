...
import { getSettings, updateSettings, checkDimension, rebuildCollections, clearAllData, testConnection, subscribeRebuildProgress, getRebuildStatus, getDependencyHealth, type SettingsResponse, type TestConnectionResult, type DimensionCheckResult, type RebuildProgressEvent, type OptionalModelStatus } from "../../api/settings";
import { useToastStore } from "../../stores/toastStore";
import { CheckIcon } from "../shared/Icons";
import { Skeleton } from "../shared/Skeleton";
import { PasswordChangePanel } from "../auth/PasswordChangePanel";
...
export function SettingsPage() {
  ...
  const [optionalModels, setOptionalModels] = useState<Record<string, OptionalModelStatus>>({});
  ...
  useEffect(() => {
    ...
    const refreshModels = () => {
      getDependencyHealth().then((r) => setOptionalModels(r.optional_models || {})).catch(() => {});
    };
    refreshModels();
    const modelTimer = setInterval(refreshModels, 5000);
  ...
  return (
    <div className="chat-main">
      ...
      <div className="settings-content">
        ...
        <div className="settings-section">
          <div className="settings-section-title">可选模型状态</div>
          {Object.entries(optionalModels).map(([name, model]) => (
            <div className="settings-field" key={name} style={{ marginTop: 10 }}>
              <label>{name === "ocr" ? "OCR" : "Reranker"}：{model.status}</label>
              <span className="settings-hint">{model.message}</span>
              {model.last_error && <span className="settings-hint" style={{ color: "var(--danger)" }}>{model.last_error}</span>}
              {(model.status === "failed" || model.status === "missing_dependency") && (
                <code style={{ display: "block", marginTop: 6, fontSize: 11 }}>{model.manual_command}</code>
              )}
            </div>
          ))}
          {!Object.keys(optionalModels).length && <span className="settings-hint">正在读取模型状态…</span>}
        </div>
