...
```bash
cd backend
pytest tests/ -v              # 298 个单元/集成测试
python tests/evaluate_rag.py  # 检索精准度评测（含消融对比）
...
For local development, see the [中文](#中文) section above.

### Evaluation

Business eval results (chunk_size=200, 25 queries across 4 domains):
...
```bash
cd backend
pytest tests/ -v              # 298 unit/integration tests
